...
**RedisForge** -- 基于 GoFrame v2 + MySQL + Redis 的后端实验项目，演示 JWT 认证、Token 黑名单、验证码 TTL、用户资料缓存（Cache Aside）、团队成员集合、在线状态心跳、团队动态流等功能。

模块名：`redis-demo`，Go 1.25.0，服务端口 `:8000`。
...
- **internal/controller/** -- 薄层 HTTP 处理器，从 JWT 上下文中提取用户信息后委托给 logic。每个操作一个文件：`{name}_v1_{action}.go`
...
- 获取当前用户 ID：`g.RequestFromCtx(ctx).GetCtxVar("userId").Uint()`
