...
### 当前下一步：README 与演示脚本整理

下一步整理 README 或演示脚本，串联登录、团队、任务、通知、动态、排行和限流主流程。
...
已在 `UpdateStatus` 中触发 `task_status_updated`：仅当任务状态实际变化、当前负责人非零且不是操作者本人时创建通知，并继续复用已完成的列表、未读数与已读链路。

已验证：`gofmt -l internal/logic/task/task.go internal/logic/notification/notification.go` 无输出，`go test ./internal/logic/task ./internal/logic/notification -count=1` 通过。

### 已完成：状态通知自动测试补强

已补充 `UpdateStatus` 自动测试，覆盖状态变化时向负责人创建 `task_status_updated` 通知并写入 Redis 未读集合、状态不变时不重复写动态/热度/通知、操作者就是负责人时不通知自己。

已验证：`go test ./internal/logic/task ./internal/logic/notification -count=1` 通过。

### 已完成：状态通知 HTTP 联调

已通过真实 HTTP 链路验证：`PATCH /tasks/{taskId}/status` 成功触发 `task_status_updated` 通知，负责人可通过 `GET /notifications` 看到未读通知，`GET /notifications/unread-count` 返回新增未读数，`PATCH /notifications/{notificationId}/read` 后 MySQL `is_read/read_at` 更新且 Redis 未读集合清理完成。

联调使用临时数据：任务 `2`、操作者 `userId=2`、临时负责人 `userId=5`、通知 `8`。验收后已恢复任务原状态与负责人，删除测试通知，并回退团队动态、任务热度和 Redis 未读集合。

### 已完成：通知中心后端阶段收口

通知中心后端链路已完成：任务指派通知、任务状态通知、通知列表、未读数、标记已读和 Redis 未读集合清理均已完成验证。页面可视化复验在浏览器会话可用时补做。
...
目标：收口限流阶段，并开始整理项目展示材料。

1. 梳理 README 或演示脚本，串联登录、团队、任务、通知、动态、排行和限流主流程。
2. 检查路线图与实际代码状态是否一致，准备进入锁与延迟任务等增强能力。
3. 有可用浏览器会话时，补做控制台页面通知中心可视化复验。
